...
os.mkdir("catalog_consensus")
os.chdir("catalog_consensus")

for catalog in catalogs:
    region_file = open(catalog.name+".fa","w")
    for sheep, consensus in catalog.consensus_seq.items():
        region_file.write(">"+(sheep)+"\n")
        counter = 0
        for i in range(60,len(consensus),60):
            region_file.write(consensus[counter:i]+"\n")
            counter += 60
    region_file.close()
